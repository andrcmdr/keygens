...
# ECDSA NIST P-256, P-384, P-521 bits
openssl genpkey -algorithm EC -pkeyopt ec_paramgen_curve:P-521 -outform PEM -out ${user:+"${user}."}${resource:+"${resource}."}key.pem -aes-256-cbc -pass env:pwd
...
ssh-keygen -f ${user:+"${user}."}${resource:+"${resource}."}key.ssh -t ecdsa -b 521 -P "${pwd}" -C "${email}"
