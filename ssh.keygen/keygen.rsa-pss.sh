...
# RSA, RSA-PSS
openssl genpkey -algorithm RSA-PSS -pkeyopt rsa_keygen_bits:4096 -pkeyopt rsa_keygen_primes:4 -pkeyopt rsa_pss_keygen_md:sha512 -pkeyopt rsa_pss_keygen_mgf1_md:sha512 -outform PEM -out ${user:+"${user}."}${resource:+"${resource}."}key.pem -aes-256-cbc -pass env:pwd
...
ssh-keygen -f ${user:+"${user}."}${resource:+"${resource}."}key.ssh -t rsa -b 4096 -P "${pwd}" -C "${email}"
sed -r -s -i"" "s/==.*$/== ${email}/gI" ${user:+"${user}."}${resource:+"${resource}."}key.ssh.pub
