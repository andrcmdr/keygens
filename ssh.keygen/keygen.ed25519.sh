...
openssl genpkey -algorithm ED25519 -outform PEM -out ${user:+"${user}."}${resource:+"${resource}."}key.pem -aes-256-cbc -pass env:pwd
...
ssh-keygen -f ${user:+"${user}."}${resource:+"${resource}."}key.ssh -t ed25519 -b 256 -P "${pwd}" -C "${email}"
